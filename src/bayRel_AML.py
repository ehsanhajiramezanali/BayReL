...
adj_s = np.load('data/drug/adj_drug.npy', allow_pickle=True)
adj_s_sp = sp.csr_matrix(adj_s, dtype=np.float64)
feat_s = np.load('data/drug/feat_drug.npy', allow_pickle=True)
# feat_s_sp = sp.lil_matrix(feat_s)

adj_t = np.load('data/drug/adj_gene_db.npy', allow_pickle=True)
adj_t_sp = sp.csr_matrix(adj_t, dtype=np.int64)
feat_t = np.load('data/drug/feat_gene_db.npy', allow_pickle=True)
...
source_id_dict = np.load('data/drug/dict_drug.npy', allow_pickle=True).tolist()
target_id_dict = np.load('data/drug/dict_gene_db.npy', allow_pickle=True).tolist()
val_intracts_df = pd.read_csv('data/drug/valIntDgidb.txt', delimiter='\t')
...
class_df = pd.read_csv('data/drug/drugClasses.txt', delimiter='\t', header=None)
class_dict = {}
for index, item in enumerate(np.unique(class_df.iloc[:,1].values)):
    class_dict[item] = index

class_data = 100*np.ones(num_nodes_s)
for i in range(class_df.shape[0]):
    class_data[source_id_dict[class_df.iloc[i, 0]]] = class_dict[class_df.iloc[i, 1]]

class_node_dict = {}
all_classes = np.unique(class_data)
num_classes = all_classes.shape[0]-1
for i in range(all_classes.shape[0]-1):
    class_node_dict[i] = np.where(class_data==i)[0]


def single_class(biadj, class_node_dict):
    dc_scores = []
    di_scores = []
...
for epoch in range(epochs):
    ...
    for threshold in range(30, 90):
        ...
        if deg <= 0.5 and pv >= 0.1:
            class_vals = single_class(post_adj, class_node_dict)
            best_vals.append([epoch, threshold, pv, deg, class_vals[0], class_vals[1]])
    ...
    for i in range(len(best_vals)):
        print("Epoch:", '%04d' % (best_vals[i][0] + 1)
              ...
              ,"dc/dcm=", "{:.3f}".format(best_vals[i][4])
              ,"dc/dci=", "{:.3f}".format(best_vals[i][5]))
        print('---------------------------------')
    print('---------------------------------')
    print('---------------------------------')
    print('---------------------------------')
