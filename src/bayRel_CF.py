...
adj_s = np.load('data/CF/adj_mirCF.npy', allow_pickle=True)
adj_s_sp = sp.csr_matrix(adj_s, dtype=np.int64)
feat_s = np.load('data/CF/feat_mirCF.npy', allow_pickle=True)

adj_t = np.load('data/CF/adj_metCF.npy', allow_pickle=True)
adj_t_sp = sp.csr_matrix(adj_t, dtype=np.int64)
feat_t = np.load('data/CF/feat_metCF.npy', allow_pickle=True)
...
target_id_dict = np.load('data/CF/dict_metCF.npy', allow_pickle=True).tolist()
source_id_dict = np.load('data/CF/dict_mirCF.npy', allow_pickle=True).tolist()
val_intracts_df = pd.read_csv('data/CF/valInteractions.txt', delimiter='\t')

val_interacts = []
for i in range(val_intracts_df.shape[0]):
    val_interacts.append([source_id_dict[val_intracts_df.iloc[i,1]], target_id_dict[val_intracts_df.iloc[i,0]]])

ng_val_src1_df = pd.read_csv('data/CF/source1.txt', delimiter='\t', header=None)
ng_val_src2_df = pd.read_csv('data/CF/source2.txt', delimiter='\t', header=None)

ng_val_src1 = []
ng_val_src2 = []
for i in range(ng_val_src1_df.shape[0]):
    ng_val_src1.append(source_id_dict[ng_val_src1_df.iloc[i,0]])

for i in range(ng_val_src2_df.shape[0]):
    ng_val_src2.append(source_id_dict[ng_val_src2_df.iloc[i,0]])
...
def neg_val(biadj, ng_val_src1, ng_val_src2):
...
#     validations = []
    ...
    for threshold in range(30, 90):
        ...
        nv = neg_val(post_adj, ng_val_src1, ng_val_src2)
        if nv >= 0.96 and pv >= 0.3:
            best_vals.append([epoch, threshold, pv, nv])
